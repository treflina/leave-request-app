...
class RequestFormView(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['form'].fields['send_to_person'].queryset = User.objects.filter(Q(id=self.request.user.manager_id)|Q(role='S')|Q(role='T')).exclude(id=self.request.user.id).order_by('-role')
...
class RequestsListView(TopManagerPermisoMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.role == "T" or user.role =="S" or user.is_staff:
            context['requests_holiday'] = Request.objects.requests_holiday_topmanager(user)
            context['requests_others'] = Request.objects.requests_others_topmanager(user)
        else:
            context['requests_holiday'] = Request.objects.requests_holiday(user)
...
class ReportView(TopManagerPermisoMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        urlop_data = [["Lp.", "Data złożenia", "Nazwisko i imię", "Od", "Do", "Status", "Podpisany przez:"]]
        other_data = [
            ["Lp.", "Data złożenia", "Nazwisko i imię", "W dniu", "Rodzaj", "Za pracę dnia", "Status",
             "Podpisany przez:"]]
        c_data = [["Lp.", "Data wystawienia", "Nr dokumentu",  "Nazwisko i imię", "Rodzaj", "Od", "Do","Inne"]]

        if type == "W":
            x = 1 
            if person == "all_employees":
                requests_data = Request.objects.filter(Q(type = 'W')&Q(start_date__gte= start_date)&Q(start_date__lte = end_date)).order_by('created').all()
                name = ""
                position=""
                ...
                requests_data = Request.objects.filter(Q(type = 'W')&Q(author__id = employee.id)&Q(start_date__gte= start_date)&Q(start_date__lte = end_date)&~Q(status = "oczekujący")).order_by('created').all()
            for item in requests_data:
                created_newformat = str(item.created).split()[0]
                employee_repr = item.author.last_name+" "+item.author.first_name+" "+item.author.position_addinfo
            ...
            create_pdf(urlop_data, pdf_buffer, title, start_date, end_date, name, position)
            pdf_buffer.seek(0)
            ...
            for item in requests_data:
                created_newformat = str(item.created).split()[0]
                employee_repr = item.author.last_name+" "+item.author.first_name+" "+item.author.position_addinfo
                data1 = [x, created_newformat, employee_repr, item.start_date, item.type, item.work_date, item.status, item.signed_by]
                other_data.append(data1)
                x = x + 1

            title = "Wnioski o dni wolne za pracujące soboty (niedziele, święta)"
            create_pdf(other_data, pdf_buffer, title, start_date, end_date, name, position)
            pdf_buffer.seek(0)
            return FileResponse(pdf_buffer, as_attachment=True,
        ...
        if type == "C":
            x = 1 
            if person == "all_employees":
                sickleaves_data = Sickleave.objects.filter(Q(start_date__gte= start_date)&Q(start_date__lte = end_date)).order_by('start_date').all()
                name = ""
                position=""
                employee = "- wszyscy pracownicy"
            else:
                ...
                name = employee.last_name + " " + employee.first_name
                position = employee.position
                sickleaves_data = Sickleave.objects.filter(Q(employee__id = employee.id)&Q(start_date__gte= start_date)&Q(start_date__lte = end_date)).order_by('start_date').all()
            for item in sickleaves_data:
                employee_repr = item.employee.last_name + " " + item.employee.first_name + " " +item.employee.position_addinfo
                data2 = [x, item.issue_date, item.doc_number, employee_repr, item.type, item.start_date, item.end_date, item.additional_info]
                c_data.append(data2)
                x = x + 1
